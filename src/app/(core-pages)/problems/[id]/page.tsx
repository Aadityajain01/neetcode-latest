...
// ... (Keep your CONSTANTS and TEMPLATES here as they were) ...
// --- TEMPLATES & CONSTANTS ---
...
export default function ProblemDetailPage() {
  ...
  // FIX: Using a view mode to decide what to show in the output window
  // "run" = Show Run Output, "submit" = Show Submission Result
  ...
  useEffect(() => {
    ...
    // Safety check: Don't poll if we are in "Run" mode
    if (outputMode === 'run') return;
  ...
  }, [currentSubmission?._id, outputMode]); // Added outputMode dependency
  ...
  // --- 4. REFACTORED RUN CODE ---
  const handleRunCode = async () => {
    if (!problem) return;
    
    // 1. Reset States explicitly
    setIsRunning(true);
    setOutput("Running...");
    setCurrentSubmission(null); // CRITICAL: Clear any previous submission state
    setOutputMode('run');       // CRITICAL: Tell UI we are in Run mode
    setActiveTab('output');
    ...
    try {
      ...
      for (let i = 0; i < 20; i++) {
        ...
        // Status 1=In Queue, 2=Processing. Anything else is done.
  ...
  // --- 5. REFACTORED SUBMIT CODE ---
  const handleSubmitCode = async () => {
    if (!problem || !problemId) return;
    
    // 1. Reset States explicitly
    setIsSubmitting(true);
    setOutput("");             // CRITICAL: Clear Run output
    setOutputMode('submit');   // CRITICAL: Tell UI we are in Submit mode
    setActiveTab('output');
    ...
    try {
      ...
      setOutputMode(null); // Revert mode on failure
    } finally { 
          ...
          <div className="flex-1 flex flex-col bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden relative shadow-sm">
             ...
             <div className="flex-1 relative bg-[#1e1e1e]">
               ...
               <div className="absolute bottom-4 right-6 flex gap-2 z-10">
                 ...
                 {currentSubmission?.status === 'accepted' && outputMode === 'submit' && (
             ...
             <div className="flex-1 p-0 overflow-hidden relative">
                {activeTab === 'custom_input' ? (
                  ...
                  <div className="w-full h-full p-4 font-mono text-sm overflow-auto text-zinc-300 custom-scrollbar relative">
                     ...
                     {/* CONDITIONAL RENDERING LOGIC FIXED HERE */}
