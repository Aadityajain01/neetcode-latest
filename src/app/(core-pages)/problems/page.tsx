...
export default function ProblemsPage() {
  ...
  return (
    <MainLayout>
      <div className="h-[calc(100vh-80px)] max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-4 md:py-6 flex flex-col font-sans animate-in fade-in slide-in-from-bottom-4 duration-500">
        
        {/* Header & Filters (Sticky/Fixed Top) */}
        <div className="flex flex-col gap-4 shrink-0 mb-4 bg-zinc-900/40 rounded-3xl p-4 md:p-6 border border-zinc-800/50 backdrop-blur-md shadow-sm relative overflow-hidden">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 relative z-10">
            <div className="space-y-1">
              ...
              <h1 className="text-3xl md:text-4xl font-black text-white tracking-tight leading-tight">Problems Archive</h1>
            ...
            <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2 p-1.5 bg-zinc-950/80 border border-zinc-800/80 rounded-2xl shadow-inner w-full md:w-auto">
              ...
              <div className="h-6 w-[1px] bg-zinc-800 hidden sm:block shrink-0"></div>
              ...
              <div className="w-full sm:w-auto px-1 pb-1 sm:pb-0 sm:px-1 shrink-0">
                ...
                >
                  <SelectTrigger className="w-full sm:w-[130px] bg-zinc-900 border border-zinc-700/80 rounded-xl h-10 text-zinc-300 hover:bg-zinc-800 transition-colors focus:ring-1 focus:ring-emerald-500 shadow-none text-sm">
        ...
        <div className="flex-1 flex flex-col min-h-0 bg-zinc-900/30 backdrop-blur-xl border border-zinc-800/60 rounded-[2rem] overflow-hidden shadow-xl">
          ...
          {/* Optional Table Header */}
           <div className="hidden sm:grid grid-cols-12 gap-4 px-8 py-3.5 text-[10px] font-black text-zinc-500 uppercase tracking-widest bg-zinc-950/80 border-b border-zinc-800/60 shrink-0">
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="h-full flex flex-col items-center justify-center p-10 min-h-[300px]">
                <div className="h-20 w-20 rounded-[1.5rem] bg-zinc-900/50 flex items-center justify-center mb-5 border border-zinc-800 shadow-inner">
                ...
                {problems.map((problem) => {
                  ...
                  return (
                    <Link 
                      ...
                      className="group relative flex flex-col sm:flex-row sm:items-center justify-between p-4 md:px-8 md:py-4 hover:bg-zinc-800/40 transition-all duration-300 gap-4 sm:gap-0"
                    >
                      ...
                      <div className="flex items-start sm:items-center gap-4 sm:gap-5 min-w-0 flex-1 relative z-10 w-full">
                        ...
                        <div className="shrink-0 pt-0.5 sm:pt-0">
                          {isSolved ? (
                            <div className="flex items-center justify-center h-10 w-10 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-500 shadow-[0_0_15px_rgba(16,185,129,0.15)] group-hover:bg-emerald-500/20 transition-colors">
                              <CheckCircle2 className="h-5 w-5" />
                            </div>
                          ) : (
                            <div className="flex items-center justify-center h-10 w-10 rounded-xl bg-zinc-900 border border-zinc-800 text-zinc-500 group-hover:border-zinc-700 group-hover:text-zinc-400 group-hover:bg-zinc-800 shadow-inner transition-all">
                        ...
                        <div className="min-w-0 flex-1">
                          <h3 className="font-bold text-zinc-200 group-hover:text-emerald-400 transition-colors text-[15px] md:text-base truncate pr-4">
                            {problem.title}
                          </h3>
                          {problem.tags && problem.tags.length > 0 && (
                            <div className="flex flex-wrap gap-1.5 mt-1.5 hidden sm:flex">
                              {problem.tags.slice(0, 3).map(tag => (
                                <Badge key={tag} variant="secondary" className="bg-zinc-950 border-zinc-800 font-bold text-zinc-500 text-[8px] px-2 py-0.5 h-auto uppercase tracking-widest hover:bg-zinc-800 hover:text-zinc-300 transition-colors shadow-sm">
                                  {tag}
                                </Badge>
                              ))}
                            </div>
          ...
          {/* Pagination Details (Sticky Bottom of Table) */}
          {totalProblems > 0 && (
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 p-4 md:px-8 border-t border-zinc-800/60 bg-zinc-950/90 backdrop-blur-xl shrink-0">
              <p className="text-[10px] font-bold text-zinc-500 uppercase tracking-widest">
                ...
                <span className="text-emerald-400">{totalProblems}</span> Valid Problems
              </p>
              ...
              <div className="flex items-center gap-1.5">
                <Button
                  ...
                  className="h-8 w-8 bg-zinc-900 border-zinc-800 text-zinc-400 hover:bg-zinc-800 hover:text-white rounded-[10px] active:scale-95 transition-all focus:ring-1 focus:ring-emerald-500 shadow-sm"
                >
